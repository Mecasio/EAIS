...
router.get("/get_student_number", async (req, res) => {
  try {
    const [rows] = await db3.query(`
        SELECT sts.student_number, pt.person_id, sts.year_level_id, pt.campus, ct.curriculum_id, sy.year_id, sy.semester_id FROM student_status_table sts
