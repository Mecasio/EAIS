  ...
  return (
    <Box sx={{ height: "calc(100vh - 150px)", overflowY: "auto", padding: 2, mt: 1 }}>
      ...
      <Box sx={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
        {/* Form */}
        <Box sx={{ flex: 1, p: 3, bgcolor: "#fff", border: `2px solid ${borderColor}`, boxShadow: 2, width: 800, minWidth: "300px" }}>
        ...
        {/* Table */}
        <Box sx={{ flex: 1, p: 3, bgcolor: "#fff", boxShadow: 2, border: `2px solid ${borderColor}`,  minWidth: "300px" }}>
          <Typography variant="h6" mb={2} style={{ color: subtitleColor }}>
            Saved School Years
          </Typography>
              ...
              <tbody>
                {filteredSchoolYears.length > 0 ? filteredSchoolYears.map((sy, index) => (
                  <tr key={index} style={{ backgroundColor: sy.astatus === 1 ? "#d4edda" : "transparent", color: sy.astatus === 1 ? "#155724" : "inherit" }}>
                    <td className="p-2 text-center" style={{ border: `2px solid ${borderColor}` }}>{index + 1}</td>
